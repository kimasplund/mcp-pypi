...
async def test_unknown_method(rpc_server):
    ...
    request = {"jsonrpc": "2.0", "method": "nonexistent_method", "id": 1}
...
async def test_method_with_error_response(rpc_server, mock_client):
    ...
    error_data = {"error": {"code": "not_found", "message": "Package not found"}}
...
async def test_no_params_handling(rpc_server, mock_client):
    ...
    mock_client.get_newest_packages.return_value = [
        {"name": "package1"},
        {"name": "package2"},
    ]

    request = {"jsonrpc": "2.0", "method": "get_newest_packages", "id": 1}
...
async def test_process_mcp_stdin():
    ...
    with (
        patch("mcp_pypi.cli.server.RPCServer", return_value=mock_server),
        ...
        mock_print.assert_called_once_with(
            '{"jsonrpc": "2.0", "result": "success", "id": 1}', flush=True
        )
...
async def test_start_server():
    ...
    # Set up patching
    with (
        patch.dict("sys.modules", {"aiohttp.web": mock_web}),
        patch("mcp_pypi.cli.server.PyPIClient", return_value=mock_client),
