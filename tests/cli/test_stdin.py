    ...
    for i, request in enumerate(requests):
        ...
        try:
            ...
            if "result" in response_obj:
                if (
                    isinstance(response_obj["result"], dict)
                    and "tools" in response_obj["result"]
                ):
                    response_obj["result"][
                        "tools"
                    ] = f"[{len(response_obj['result']['tools'])} tools]"
                elif (
                    isinstance(response_obj["result"], dict)
                    and "results" in response_obj["result"]
                ):
                    response_obj["result"][
                        "results"
                    ] = f"[{len(response_obj['result']['results'])} results]"
